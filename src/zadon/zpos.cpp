...
static const CBlockIndex* FindIndexFrom(uint32_t nChecksum, libzerocoin::CoinDenomination denom, int cpHeight)
{
    ...
    // Height limits are ensured by the contextual checks in NewZAdonStake
...
CLegacyZAdonStake* CLegacyZAdonStake::NewZAdonStake(const CTxIn& txin, int nHeight)
{
    ...
    if (!txin.IsZerocoinSpend()) {
        LogPrintf("%s: unable to initialize CLegacyZAdonStake from non zc-spend", __func__);
        return nullptr;
    }
    ...
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) ||
        ...
        LogPrint(BCLog::LEGACYZC, "%s : zADON stake block: height %d outside range", __func__, nHeight);
        return nullptr;
    }
    ...
    if (_pindexFrom == nullptr) {
        LogPrintf("%s : Failed to find the block index for zadon stake origin", __func__);
        return nullptr;
    }

    // All good
    return new CLegacyZAdonStake(_pindexFrom, _nChecksum, _denom, _hashSerial);
}
