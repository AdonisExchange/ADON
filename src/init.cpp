 ...
 */
const char * const ADON_PID_FILENAME = "adon.pid";

fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", ADON_PID_FILENAME));
...
void Shutdown()
{
    ...
    util::ThreadRename("adon-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", ADON_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", ADON_PID_FILENAME));
#endif
...
{
    return FormatParagraph(_("Copyright (C) 2009-2021 The Bitcoin Core Developers")) + "\n" +
           "\n" +
           FormatParagraph(_("Copyright (C) 2014-2021 The Dash Core Developers")) + "\n" +
           "\n" +
           FormatParagraph(_("Copyright (C) 2015-2021 The PIVX Core Developers")) + "\n" +
           "\n" +
...
{
    util::ThreadRename("adon-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that ADON is running in a usable environment with all
...
{
    // Make sure only a single ADON process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", ADON_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if ADON is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if ADON is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //ADON specific: zerocoin and spork DB's
                ...
                // ADON: load previous sessions sporks if we have them.
