...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ADON);
    ...
    if (settings.contains("nAnonymizeAdonAmount") || reset)
        gArgs.SoftSetArg("-anonymizeadonamount", settings.value("nAnonymizeAdonAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in adon.cpp during shotdown to also remove the window positions
