...
#include "qt/adon/forms/ui_snackbar.h"
#include "qt/adon/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ADONGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ADONGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
