// Copyright (c) 2019 The ADON developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/adon-config.h"
#endif

#include "qt/adon/settings/settingsfaqwidget.h"
#include "qt/adon/settings/forms/ui_settingsfaqwidget.h"
#include "qt/adon/mnmodel.h"
#include "qt/adon/qtutils.h"

#include <QScrollBar>
#include <QMetaObject>

SettingsFaqWidget::SettingsFaqWidget(ADONGUI* parent, MNModel* mnModel) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableADON,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableADON,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableADON,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("ADON is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("ADON utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableADONContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received ADON requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your ADON wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableADON->setText(unspendableADONContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of ADON with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("adon-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.adonis.network'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(mnModel->getMNCollateralRequiredAmount(), BitcoinUnits::ADON)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in ADON Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the ADON network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableADON, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableADON);});
    ...
    if (parent)
        connect(parent, &ADONGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
