...
#include "qt/adon/forms/ui_addresseswidget.h"
#include "qt/adon/addressfilterproxymodel.h"
#include "qt/adon/addresslabelrow.h"
...
AddressesWidget::AddressesWidget(ADONGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto adonAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(adonAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(adonAdd);
        if (walletModel->updateAddressBookLabels(adonAdd, label.toUtf8().constData(),
