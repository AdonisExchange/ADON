...
#include "qt/adon/send.h"
#include "qt/adon/forms/ui_send.h"
#include "qt/adon/addnewcontactdialog.h"
#include "qt/adon/qtutils.h"
#include "qt/adon/sendchangeaddressdialog.h"
#include "qt/adon/optionbutton.h"
#include "qt/adon/sendconfirmdialog.h"
#include "qt/adon/guitransactionsutils.h"
#include "qt/adon/loadingdialog.h"
...
SendWidget::SendWidget(ADONGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a ADON URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-adon");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onADONSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onADONSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent ADONs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield ADONs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent ADONs to shield."));
    }
}
...
void SendWidget::onADONSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto adonAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(adonAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(adonAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(adonAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
