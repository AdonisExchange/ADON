// Copyright (c) 2019-2020 The ADON developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "qt/adon/adongui.h"
...
#include "qt/adon/qtutils.h"
#include "qt/adon/defaultdialog.h"
#include "shutdown.h"
...
const QString ADONGUI::DEFAULT_WALLET = "~Default";

ADONGUI::ADONGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void ADONGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &ADONGUI::toggleHidden);
...
void ADONGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &ADONGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &ADONGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &ADONGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &ADONGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &ADONGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &ADONGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &ADONGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &ADONGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &ADONGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &ADONGUI::execDialog);
    connect(governancewidget, &GovernanceWidget::showHide, this, &ADONGUI::showHide);
    connect(governancewidget, &GovernanceWidget::execDialog, this, &ADONGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &ADONGUI::execDialog);
}


void ADONGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void ADONGUI::handleRestart(QStringList args)
{
...
void ADONGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &ADONGUI::message);
...
void ADONGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &ADONGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &ADONGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void ADONGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void ADONGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void ADONGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &ADONGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &ADONGUI::show);
                e->ignore();
            }
        }
    }
...
void ADONGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void ADONGUI::messageInfo(const QString& text)
{
...
void ADONGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool ADONGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void ADONGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void ADONGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ADONGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void ADONGUI::goToSend()
{
    showTop(sendWidget);
}

void ADONGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void ADONGUI::goToMasterNodes()
{
    showTop(masterNodesWidget);
}

void ADONGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void ADONGUI::goToGovernance()
{
    showTop(governancewidget);
}

void ADONGUI::goToSettings(){
    showTop(settingsWidget);
}

void ADONGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void ADONGUI::goToReceive()
{
    showTop(receiveWidget);
}

void ADONGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void ADONGUI::showTop(QWidget* view)
{
...
void ADONGUI::changeTheme(bool isLightTheme)
{
...
void ADONGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool ADONGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void ADONGUI::showHide(bool show)
{
...
int ADONGUI::getNavWidth()
{
    return this->navMenu->width();
}

void ADONGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
void ADONGUI::setGovModel(GovernanceModel* govModel)
{
    if (!stackedContainer || !clientModel) return;
    governancewidget->setGovModel(govModel);
}

void ADONGUI::setMNModel(MNModel* _mnModel)
{
    if (!stackedContainer || !clientModel) return;
    mnModel = _mnModel;
...
bool ADONGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &ADONGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &ADONGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &ADONGUI::message);
    connect(topBar, &TopBar::message, this, &ADONGUI::message);
    connect(sendWidget, &SendWidget::message,this, &ADONGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &ADONGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &ADONGUI::message);
    connect(governancewidget, &GovernanceWidget::message,this, &ADONGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &ADONGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &ADONGUI::incomingTransaction);

    return true;
}

bool ADONGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void ADONGUI::removeAllWallets()
{
    // Single wallet supported.
}

void ADONGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(ADONGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ADONGUI::subscribeToCoreSignals()
{
...
void ADONGUI::unsubscribeFromCoreSignals()
{
