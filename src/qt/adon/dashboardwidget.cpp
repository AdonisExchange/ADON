...
#include "qt/adon/dashboardwidget.h"
#include "qt/adon/forms/ui_dashboardwidget.h"
#include "qt/adon/sendconfirmdialog.h"
#include "qt/adon/txrow.h"
...
DashboardWidget::DashboardWidget(ADONGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareAdon, "square-chart-adon");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelAdon, "text-chart-adon");
    ...
    setCssProperty(ui->labelAmountAdon, "text-stake-adon-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &ADONGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("ADON")
    updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(74, 117, 163);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isAdon = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair ADON, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            adon = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalAdon += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesAdon.append(adon);
        chartData->valuesMN.append(mn);

        int max = std::max(adon, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(249, 180, 16));
    set1->setColor(QColor(255, 219, 69));
    ...
    set0->append(chartData->valuesAdon);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalAdon > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountAdon, "text-stake-adon");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountAdon, "text-stake-adon-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountAdon, ui->labelAmountMN});
    ui->labelAmountAdon->setText(GUIUtil::formatBalance(chartData->totalAdon, nDisplayUnit));
    ...
    if (fShowCharts) {
        if (!stakesFilter) {
            ...
            stakesFilter->setTypeFilter(TransactionFilterProxy::TYPE(TransactionRecord::StakeMint) |
                                        ...
                                        TransactionFilterProxy::TYPE(TransactionRecord::StakeZADON) |
