...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select ADON Outputs to Spend" :
            "Select Shielded ADON to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::ADON;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show ADON address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
