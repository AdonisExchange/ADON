...
#include "qt/adon/splash.h"
#include "qt/adon/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/adon/governancemodel.h"
...
{
    return QCoreApplication::translate("adon-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in adon.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in adon.qrc)
...
/** Class encapsulating ADON Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (ADONGUI)
...
{
    window = new ADONGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &ADONGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &ADONGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(ADONGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(ADONGUI::DEFAULT_WALLET);
        ...
        // ADON: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ADONGUI::handlePaymentRequest);
        connect(window, &ADONGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. ADON can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(adon_locale);
    Q_INIT_RESOURCE(adon);
    ...
    /// 6. Determine availability of data directory and parse adon.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", ADON_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // adon: links repeatedly have their payment requests routed to this process:
