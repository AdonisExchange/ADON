...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("ADONMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("ADONMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("ADONMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("adon-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ADONMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ADONMiner exception");
    } catch (...) {
        LogPrintf("ADONMiner exception");
    }

    LogPrintf("ADONMiner exiting\n");
}
