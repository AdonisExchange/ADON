...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " adon-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  adon-tx [options] <hex-tx> [commands]  Update hex-encoded adon transaction\n" +
                               "or:     adon-tx [options] -create [commands]   Create hex-encoded adon transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded adon transaction
