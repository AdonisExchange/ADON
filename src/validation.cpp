...
#if defined(NDEBUG)
#error "ADON cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uadon) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k ADON
...
{
    util::ThreadRename("adon-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC_V2) &&
        ...
        if (pindex->nHeight % 50000 == 0) {
            ZADONModule::CleanCoinSpendsCache();
        }
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Adon Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nAdonBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nAdonBadBlockBits)) {
            // accept ADON block minted with incorrect proof of work threshold
            return true;
        }
                                     ...
                                     std::set<CBigNum>& spent_serials)
{
    ...
    const bool zadonActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC);
    const bool publicZadonActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_PUBLIC);
    ...
    for (size_t i = 1; i < block.vtx.size(); i++) {
        ...
        for (const CTxIn& in: tx->vin) {
            ...
            if (isPublicSpend && (!publicZadonActive || v5Active)) {
            ...
            if (isPrivZerocoinSpend && (!zadonActive || publicZadonActive)) {
            ...
            if (isPrivZerocoinSpend || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZADONModule::ParseZerocoinPublicSpend(in, *tx, state, publicSpend)){
                        return false;
                    }
                    spend = publicSpend;
                } else {
                    spend = ZADONModule::TxInToZerocoinSpend(in);
                }
        ...
        for (auto it = bl.vtx.rbegin(); it != bl.vtx.rend(); ++it) {
            ...
            for (const CTxIn& in: tx->vin) {
                ...
                if (!in.IsZerocoinSpend()) {
                    ...
                    // zerocoin serial
                    const CBigNum& s = ZADONModule::TxInToZerocoinSpend(in).getCoinSerialNumber();
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, const FlatFilePos* dbp) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (isPoS) {
        ...
        if (coinstake_in.IsZerocoinSpend()) {
            libzerocoin::CoinSpend spend = ZADONModule::TxInToZerocoinSpend(coinstake_in);
            if (!ContextualCheckZerocoinSpend(coinstake, &spend, pindex->nHeight)) {
                return state.DoS(100,error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                        coinstake.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zadon");
            }
        }
