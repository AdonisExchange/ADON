...
const char * const ADON_CONF_FILENAME = "adon.conf";
const char * const ADON_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "adond -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\ADON
// Windows >= Vista: C:\Users\Username\AppData\Roaming\ADON
// Mac: ~/Library/Application Support/ADON
// Unix: ~/.adon
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ADON";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ADONParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ADONParams
    // Mac: ~/Library/Application Support/ADONParams
    // Unix: ~/.adon-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ADONParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "ADONParams";
#else
    // Unix
    return pathRet / ".adon-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/adon/sapling-spend.params";
        sapling_output = "/usr/share/adon/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/adon/sapling-spend.params";
            sapling_output = "/usr/local/share/adon/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", ADON_MASTERNODE_CONF_FILENAME));
