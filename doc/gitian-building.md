...
*Setup instructions for a gitian build of ADON Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the ADON
...
to the ADON GitHub release page.
...
- [Building ADON Core](#building-adon-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/adon-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/adon-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/adon-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/adon-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/adon-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/adon-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/AdonisNetwork/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@adon.adonis.network) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/adon-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/adon-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/adon-osx-*-build.assert
```
