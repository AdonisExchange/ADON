ADON Core version *3.1.0* is now available from:  <https://github.com/AdonisNetwork/ADON/releases>
...
ADON Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zADON Staking is here! zADON staking will be activated on the morning of the 8th of May 2018. With the release of zADON staking, there are effectively 2 versions of zADON, zADON minted on the 3.0.6 ADON wallet or lower, and zADON minted on ADON wallet version or higher. New features in this release will require the use of zADON v2, zADON minted on this wallet release 3.1.0 or later. If you currently hold zADON v1 and wish to take advantage of zADON staking and deterministic zADON, you will need to spend the zADON v1 to yourself and remint zADON v2.
Note: To find your zADON version, click the privacy tab, then the zADON Control button then expand the arrows next to the desired denomination.


### Deterministic zADON Seed Keys

zADON is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zADON outside of the wallet that the zADON had been minted on. zADON can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zADON minting

zADON minting now only requires 1 further mint (down from 2) to mature. zADON mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zADON Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zADON denomination has been spent or not.



ADON/zADON Staking and Masternode Rewards
--------------

### ADON, zADON and Masternode Payment Schedule

To encourage the use of zADON and increase the ADON zerocoin anonymity set, the ADON payment schedule has been changed to the following:

If a user staking zADON wins the reward for their block, the following zADON reward will be: 
- 3 zADON (3 x 1 denominations) rewarded to the staker, 2 ADON rewarded to the masternode owner and 1 ADON available for the budget. This is a total block reward of 6 ADON, up from 5.

If a user staking ADON wins the reward, the following amounts will be rewarded: 
- 2 ADON to the ADON staker, 3 ADON to the Masternode owner and 1 ADON available for the budget. This is a total block reward of 6 ADON, up from 5.


### Return change to sender when minting zADON

Previously, zADON minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most ADON and return the change from a zADON mint to that address.
...
The visual layout of the ADON Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zADON minting feature. This is enabled by default and the enablezeromint=0 setting in the adon.conf file will overwrite the GUI option.
-	The percentage of autominted zADON can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular ADON.
...
### Summary
 ...
 The ADON wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the adon.conf file, found in the ADON data directory.
- backuppath = <directory / full path>
- zadonbackuppath = <directory / full path>
...
* If zADON backup, auto generated name is `wallet-autozadonbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
ADON Daemon & Client (RPC Changes)
--------------
...
This allows running adond without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the ADON support group, which often times sees users say "I didn't spend that zADON". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the ADON masternodes, the monthly budget available to be utilised has been increased to 42,000 ADON / month. This ADON only has the opportunity to be raised once per month (paid to winning proposals) with any unused ADON not created by the blockchain.

### Budget Finalization Fee

The ADON finalization fee for successful proposals has now been reduced, this fee is now 5 ADON down from 50 ADON. The total fee outlay for a successful proposal is now a total of 55 ADON.
...
-	Enforced v1 zADON spends to require a security level of 100
-	Updates to zADON spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the ADON-Qt masternodes tab
-	Updated ADON-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and ADON's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zADON Control dialog in the QT wallet, a user is able to see zADON marked as version 1 or 2.

zPoS (zADON staking)
--------------

Once a zADON has over 200 confirmations it becomes available to stake. Staking zADON will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zADON. So for example if a 1,000 zADON denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zADONs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within ADON's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zADON, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zADON that can be recalculated at any time using the seed. Deterministic zADON allows for users to backup all of their future zADON by simply recording their seed and keeping it in a safe place (similar to backing up a private key for ADON). The zADON seed needs to remain in the wallet in order to spend the zADON after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zADON data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

ADON's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which ADON used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to ADON using the bug bounty program. The modulus is now correctly imported and ADON's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zADON v2: zPoS dzADON ezADON external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zADON balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zADON status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zAdon Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault adon-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zADON spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zADON. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zADON. (presstab)
