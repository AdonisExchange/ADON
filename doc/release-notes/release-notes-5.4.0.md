ADON Core version v5.4.0 is now available from: <https://github.com/AdonisNetwork/ADON/releases>
...
If you are running an older version, shut it down. Wait until it has completely shut down, then run the installer (on Windows) or just copy over /Applications/ADON-Qt (on Mac) or adond/adon-qt (on Linux).
...
- #2579 adon-cli better error handling + libevent RAII upstream backports (furszy)
