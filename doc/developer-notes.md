...
- [Development guidelines](#development-guidelines)
    - [General ADON Core](#general-adon-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
ADON Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play ADONs (tADON)" on the test network, if you
...
ADON Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_adon
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_adon --log_level=test_suite
$ valgrind -v --leak-check=full src/adond -printtoconsole
```
...
# A coverage report will now be accessible at `./test_adon.coverage/index.html`.
```

**Sanitizers**
ADON can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates ADONs (if wallet is enabled).
...
However, in open source software such as ADON Core, where everyone uses
...
pay attention to for reviewers of ADON Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: ADON Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against ADON Core so that they can be integrated
...
file descriptors it will cause problems with ADON's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `adon-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `adon-cli` and the GUI debug console use this table to determine how to
