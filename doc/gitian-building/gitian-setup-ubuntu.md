...
The build script we'll be using is contained in the ADON github repository ([contrib/gitian-build.py](https://github.com/AdonisNetwork/ADON/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the ADON repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/AdonisNetwork/ADON/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, adon-detached-sigs, and adon GitHub repos.
